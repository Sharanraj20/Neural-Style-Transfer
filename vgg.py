...
'''
Usage:
    we only use vgg_structure().build(input_image)
...
'''
def file_vgg_read(pretrianed_vgg):
    
    mat = scipy.io.loadmat(pretrianed_vgg)
...
def pool(prev_layer):
    ...
    output = tf.nn.avg_pool(value=prev_layer, \
                            ksize=[1, 2, 2, 1], \
                            strides=[1, 2, 2, 1], \
                            padding="SAME")
    
    # if use max pooling, use this output:****************************************************
    '''
